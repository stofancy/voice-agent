...
async def test_msg_001_start_listening_clears_buffer_and_sets_state(mock_settings):
    ...
    """
    ws = MockWebSocket(receive_texts=[
        json.dumps({"type": "start_listening"}),
        json.dumps({"type": "stop_listening"}),
    ])

    with patch("src.server.main.settings", mock_settings), \
        ...
        mock_ws = create_mock_ws([json.dumps({"type": "start_listening"}),
                                   json.dumps({"type": "stop_listening"})])
...
async def test_exc_001_cancelled_error_handled(mock_settings):
    ...
    with patch("src.server.main.settings", mock_settings), \
        ...
        # 查找 tts_end 消息
        tts_end_msgs = [msg for msg in mock_ws._ws.sent_messages
...
async def test_exc_002_generic_exception_sends_fallback(mock_settings):
    ...
    with patch("src.server.main.settings", mock_settings), \
        ...
        # 查找 fallback 消息
        fallback_msgs = [msg for msg in mock_ws._ws.sent_messages
                         if "Sorry" in str(msg.get("text", ""))]
...
async def test_msg_008_perf_report_calls_log_round(mock_settings):
    ...
    with patch("src.server.main.settings", mock_settings), \
        ...
        mock_ws = create_mock_ws([
            json.dumps({"type": "perf_report", "metrics": {"button_release_to_tts_end_ms": 100}}),
        ])
